...
class ConcurrentStrategy(AntStrategy):
    ...
    def decide_action(self, perception: AntPerception) -> AntAction:
        ...
        # Initialize ant position if not yet stored
    ...
    def search_for_food(self, perception):
        ...
        if(len(self.next_orientation_list.get(ant_id, [])) != 0): # still orientation to do 
            ...
            direction_steps = random.randint(0, 2) # choose a random number between 0 and 4
    ...
    def update_position(self, perception):
        if (not self.blocked_on_something(perception)):
            ...
            # add the new position to the explored positions
            self.explored_positions[ant_id] = self.explored_positions.get(ant_id, [])
            self.explored_positions[ant_id].append(self.ant_positions[ant_id])
            if ant_id == 1:
                # print("Ant position: ", self.ant_positions[ant_id])
                # print("explored positions : ", self.explored_positions[ant_id])
                pass
    ...
    def go_forward_and_update_coordinate(self, perception):
        ...
        #print(perception.steps_taken ,"debug")
